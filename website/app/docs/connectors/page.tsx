  ...
  return (
    <>
      ...
      <Prose>
        ...
        <p>
          {siteConfig.name} ships a local MCP server named <code>folio-mcp</code>.
          It gives any MCP-aware tool <strong>read-only</strong> access to your
          transcripts, tasks, and memories over <code>stdio</code>. There is no
          ...
          <code>create_task</code>, which writes a new task back into your vault.
        </p>
      </Prose>
      ...
      <Steps>
        <Step n={1} title="Enable connectors in Folio">
          <p>
            Open <strong>Settings</strong> then <strong>Connectors</strong> and
        ...
        <Step n={2} title="Register folio-mcp with your client">
          <p>
            ...
            <code>folio-mcp</code> as a subprocess and talks to it over standard
            input and output. A minimal configuration registers a single server
            named <code>folio</code> with the command <code>folio-mcp</code> and
      ...
      <Callout variant="privacy" title="Local and read-only by design">
        <p>
          ...
          <code>create_task</code> as the only exception. The server is scoped to
