...
export default function DocsOverviewPage() {
  return (
    <>
      ...
      <Callout variant="note" title="Local by default">
        <Prose>
          <p>
            ...
            <strong>Settings</strong> then <strong>Privacy</strong>, blocks every outbound HTTP call except{" "}
            <code>localhost</code>, so you can airgap the app entirely and it still works end to end.
          </p>
        </Prose>
      </Callout>
      ...
      <Prose>
        <p>
          <strong>{siteConfig.name}</strong> is a menu-bar app for {siteConfig.platform}. When a meeting starts it records
        ...
        <p>
          ...
          <code>whisper.cpp</code> is the primary path, on-device diarization separates voices, and the notes are plain
      ...
      <Prose>
        ...
        <ul>
          <li>
            <strong>Two-stream capture.</strong> System audio and the microphone are recorded as separate streams. The
            microphone track is always labelled You.
          </li>
          <li>
            <strong>On-device transcription.</strong> The bundled backend is <code>whisper.cpp</code> through the{" "}
            <code>whisper-rs</code> bindings, Metal-accelerated on Apple Silicon. The cloud is an opt-in fallback, never
            the default.
          </li>
          <li>
            <strong>On-device diarization.</strong> Voices on the system-audio track are clustered into Speaker 1,
            Speaker 2, and so on, with no cloud involved.
          </li>
          <li>
            <strong>One markdown note per meeting.</strong> Each meeting becomes a single markdown file in your vault,
            with frontmatter for attendees, duration, model, and source.
          </li>
          <li>
            <strong>A local MCP server.</strong> <code>folio-mcp</code> gives MCP-aware tools read-only access to your
            transcripts, tasks, and memories over stdio. No cloud, no proxy.
          </li>
          <li>
            <strong>Privacy Mode.</strong> A single switch physically blocks every outbound HTTP call except{" "}
            <code>localhost</code>.
          </li>
        </ul>
      </Prose>
      ...
      <CardGrid>
        ...
        <LinkCard href="/docs/connectors" title="Connectors">
          Wire up the local <code>folio-mcp</code> server so Claude, Cursor, and other MCP tools can read your notes.
        </LinkCard>
