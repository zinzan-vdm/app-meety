...
impl Transcriber for OpenAiTranscriber {
    fn transcribe(&self, audio_path: &Path, language_hint: Option<&str>) -> Result<Transcript> {
        if self.api_key.is_empty() {
            return Err(MeetyError::Transcription(
        ...
        crate::cloud_guard::ensure_allowed(host)
            .map_err(|e| MeetyError::Transcription(e.to_string()))?;

        let client = reqwest::blocking::Client::builder()
            .timeout(REQUEST_TIMEOUT)
            .build()
            .map_err(|e| MeetyError::Transcription(format!("could not build HTTP client: {e}")))?;
        ...
        let response = client
            ...
            .multipart(form)
            .send()
            .map_err(|e| MeetyError::Transcription(format!("OpenAI request failed: {e}")))?;
        ...
        let parsed: WhisperResponse = response.json().map_err(|e| {
            MeetyError::Transcription(format!("could not parse OpenAI response: {e}"))
        })?;
