...
impl SessionTranscript {
    pub fn write_json(&self, path: &Path) -> Result<()> {
        let json = serde_json::to_string_pretty(self).map_err(|e| {
            MeetyError::Transcription(format!("could not serialize transcript: {e}"))
        })?;
        let compressed = zstd::encode_all(json.as_bytes(), ZSTD_LEVEL)
            .map_err(|e| MeetyError::Transcription(format!("zstd compress failed: {e}")))?;
...
fn read_transcript_bytes(path: &Path) -> Result<String> {
    ...
    Err(MeetyError::Transcription(format!(
