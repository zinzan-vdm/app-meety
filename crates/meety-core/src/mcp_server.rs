...
impl McpTool {
    ...
    pub fn description(self) -> &'static str {
        match self {
            McpTool::SearchMemory => "Search the user's Meety memory store (claims, prefs, people, observations) by free-text query.",
            McpTool::FindDecision => "Find decisions matching a free-text query across every recorded meeting.",
            McpTool::ListTasks => "List the user's Meety tasks filtered by optional status.",
            McpTool::CreateTask => "Create a new task in the user's Meety kanban with title + optional owner / due.",
